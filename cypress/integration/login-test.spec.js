...
describe('hacer registro en e-commerce', () => {
    ...
    it('exitoso', function () {
        ...
        cy.get('.login').click()
        cy.get('#email_create').type('juanduarte1612@gmail.com')
        cy.get('#SubmitCreate > span').click()
        cy.wait(2000)
        cy.get('#id_gender1').click()
        cy.get('#customer_firstname').type('Pepito')
        cy.get('#customer_lastname').type('Perez')
        cy.get('#passwd').type('Pepito123')
        cy.get('#address1').type('cualquier direccion')
        cy.get('#city').type('Bogota')
        cy.get('#phone_mobile').type('222546400')
    ...
    it('exitoso con variables', function () {
        cy.visit('http://automationpractice.com/index.php');
        cy.get('.login').click()
    ...
    it('fallido por código postal', function () {
        cy.visit('http://automationpractice.com/index.php');
        cy.get('.login').click()
        cy.get('#email_create').type('juanduarte3012@gmail.com')
        cy.get('#SubmitCreate > span').click()
        cy.get('#id_gender1').click()
        cy.get('#customer_firstname').type('Pepito')
        cy.get('#customer_lastname').type('Perez')
